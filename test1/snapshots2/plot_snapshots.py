    ...
    with h5py.File(filename, mode='r') as file:
        for index in range(start, start+count):
            for n, task in enumerate(tasks):
                ...
                if task=='uy':
                    dset = file['tasks'][task]
                    # plot_tools.plot_bot_3d(dset, 0, index, axes=axes, title=task, even_scale=True, visible_axes=False,clim=(-15,15))
                    plot_tools.plot_bot_3d(dset, 0, index, axes=axes, title=task, even_scale=True, visible_axes=False)
                if task=='uz':
                    dset = file['tasks'][task]
...
# ffmpeg -framerate 20 -i write_%06d.png -c:v libx264 -pix_fmt yuv420p output.mp4